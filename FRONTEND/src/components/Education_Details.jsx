...
const EducationForm = ({ onBack, onNext }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Call onNext here if the submission is successful
            onNext();
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 bg-card rounded-lg shadow-md">
            ...
            <div className="flex justify-between mt-6">
                <button type="button" className="bg-secondary text-secondary-foreground hover:bg-secondary/80 p-2 rounded-md" onClick={onBack}>BACK</button>
                <button type="submit" className="bg-destructive text-destructive-foreground hover:bg-destructive/80 p-2 rounded-md">NEXT</button>
            </div>
            <p className={`${textClasses} mt-4`}>Page 2</p>
        </form>
    );
